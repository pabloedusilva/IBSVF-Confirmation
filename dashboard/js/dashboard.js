...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadParticipants() {
        try {
            const stored = localStorage.getItem('familyDayParticipants');
    ...
    function saveParticipants() {
        try {
            localStorage.setItem('familyDayParticipants', JSON.stringify(participants));
            updateStatistics();
        } catch (error) {
    ...
    function editParticipant(id, updatedData) {
        ...
        if (index !== -1) {
            participants[index] = { ...participants[index], ...updatedData };
            saveParticipants();
            renderParticipants();
        }
    }
